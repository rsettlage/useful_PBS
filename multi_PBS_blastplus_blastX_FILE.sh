...
####qsub /groups/DAC/useful_PBS/multi_PBS_blastplus_blastX_FILE.sh -v Results=blx.txt,db_file=/groups/DAC/vector_contaminants_file/adaptors_contaminants.fa,Query=test.fa
####find `pwd` -maxdepth 1 -iname '*fasta' -exec qsub /groups/DAC/useful_PBS/multi_PBS_blastplus_blastX_FILE.sh -v Results=blstX.txt,db_file=,Query='{}' \;
...
SCRIPT=/groups/DAC/useful_PBS/multi_PBS_blastplus_blastX_FILE.sh
more $SCRIPT
module list
###########################################################################
...
###change the below to grab the pre-indexed files and then NOT do that again here
...
if [ -e $database_FILE ];then
	...
	EXEC_CMD1="makeblastdb -in $database_FILE -dbtype prot"  ###prot or nucl  <---------------set this----------####
	echo $EXEC_CMD1  
	${EXEC_CMD1}
else
...
##`blastx -outfmt '6 qseqid sseqid pident qlen slen length mismatch gapopen qstart qend sstart send evalue bitscore' -num_threads 12 -evalue 1e-6 -db $database_FILE -query $query_FILE -out $result_FILE`

##for trinity, use this format
`blastx -outfmt 6 -max_target_seqs 10 -num_threads 12 -db $database_FILE -query $query_FILE -out $result_FILE`
