...
###use find `pwd` -maxdepth 1 -iname '*accepted_hits.bam' -exec qsub /groups/DAC/useful_PBS/PBS_HTSeq_FILE.sh -v STRANDED=yes,EXT=,BAM='{}',GTF= \;
...
###########################################################################
###print PBS script
PBS_script="/groups/DAC/useful_PBS/PBS_HTSeq_FILE.sh"
echo '#############################################################################'
more $PBS_script
echo '#############################################################################'
###########################################################################
...
echo running HTSeq count script using python -m HTSeq.scripts.count -m intersection-strict -s $lib_TYPE $SAM_NAME $GTF_FILE \>$OUT_NAME
python -m HTSeq.scripts.count -m intersection-nonempty -t exon -i gene_id -s $lib_TYPE $SAM_NAME $GTF_FILE >$OUT_NAME
